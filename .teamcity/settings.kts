...
project {
    ...
    params {
        ...
        text("github_repository_name", "Javadoctor", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `Javadoctor` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "collector", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
        text("env.PUBLISHED_JAVA_GROUP", "net.neoforged.javadoctor", label = "Published group", description = "The maven coordinate group that has been published by this build. Can not be empty.", allowEmpty = false)
        ...
        text("docker_gradle_version", "8.0.1", label = "Gradle version", description = "The version of Gradle to use during execution of Gradle tasks.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
    }

    features {
        githubIssues {
            id = "Javadoctor__IssueTracker"
            displayName = "neoforged/Javadoctor"
            repositoryURL = "https://github.com/neoforged/Javadoctor"
        }
    }
}
...
object BuildSecondaryBranches : BuildType({
    ...
    id("Javadoctor__BuildSecondaryBranches")
